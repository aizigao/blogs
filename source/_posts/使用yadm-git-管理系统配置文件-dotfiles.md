...
linux 和 macos下很大部分的软件配置文件都是在用户目录对应以 “.”打开的隐藏文件或者是xxxrc 命名，另外一部分的配置也有放在`.config`目录下的。 一般来说只要备份好这些dotfiles 另写一个配置脚本，在做系统重装或系统迁移时就会比较的方便 了。

之前也一直在用 [mackup](https://github.com/lra/mackup) 做为管理工具, 但是使用下来个人感觉是不很满意

- 备份文件用的是软链接，不是很喜欢这种方式，不是很好管理, 移除mackup时会比较麻烦
- 数据同步一般用网盘或者手动git的方式，用网盘的问题在于不能做到的版本控制, 每次修改完文件后不能回滚

最近逛reddit时，发现一个 [yadm](https://yadm.io/docs/install) 的工具，作用是默认将整个用户目录处理为一个git仓库但不会影响到正常的git使用，因为方案是用git管理，好处理之类的就不用多说了，使用上也和git大体相同。
...
安装我当前的设备为一 台manjaro的系统, 使用yay为包管理
```shell
# 安装
yay  yadm
# 
...
可以看到的yadm的用法和`git`本身一毛一样，会用`git`就会用`yadm`,  因为基于git，对配置文件做点分支管理，版本回滚都很方便。并且不像mackup那样用`ln`， 入侵性很低级。


## 系统级用法 

默认yadm 控制范围是 home目录如果你想把像 `/etc` 目录下的内容也管理进入的话，我试了两个方案。
